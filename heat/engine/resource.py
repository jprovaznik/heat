...
cfg.CONF.import_opt('resource_hooks', 'heat.common.config')
...
def valid_hook(hook):
    return hook in ('pre-create', 'pre-update')
...
class Resource(object):
    ...
    def _break_if_required(self, action, status, hook):
        '''Block the resource until the hook is cleared if there is one.'''
        if not cfg.CONF.resource_hooks:
            return
        should_set_hook = self.matches_hook(
            self.stack.env.registry.as_dict().get('resources', {}),
            hook)
        if should_set_hook:
            self.state_set(action, status, "Paused until the hook is cleared")
            self.set_hook(hook)
        ...
        while self.has_hook(hook) and self.status != self.FAILED:
            try:
                ...
                self.unset_hook(hook)
                self.state_set(
                    self.INIT, self.COMPLETE,
                    "Failure occured while waiting.")

    def matches_hook(self, resources, hook):
        '''Does this resource have a hook set in the environment?
        ...
        A hook value is either a string (`pre-create` and `pre-update`) or a
        list of those.
        ...
        def user_entered_stack_name(stack):
            if stack.parent_resource:
                return stack.parent_resource.name
            else:
                return stack.name

        def matches_path(path):
            if not glob_matches(path[-1], self.name):
                return False
            current_stack = self.stack
            if current_stack.nested_depth != len(path[:-1]):
                return False
            for stack_name in reversed(path[:-1]):
                if glob_matches(stack_name,
                                user_entered_stack_name(current_stack)):
                    current_stack = current_stack.parent_resource.stack
                else:
                    return False
            return True

        def matches_hook(resource_dict):
            if 'hooks' not in resource_dict:
                return False
            hooks = resource_dict['hooks']
            return hook in hooks

        def depth_first(resources, path=None):
            if 'hooks' in resources:
                return matches_hook(resources) and matches_path(path)
            else:
                for resource_name, resource_dict in six.iteritems(resources):
                    if path is None:
                        path = []
                    new_path = path + [resource_name]
                    if depth_first(resource_dict, new_path) is True:
                        return True
                return False

        assert valid_hook(hook)
        return depth_first(resources, None)
    ...
    def create(self):
        ...
        if self.stack.action == self.stack.CREATE:
            yield self._break_if_required(
                self.CREATE, self.IN_PROGRESS, 'pre-create')
    ...
    def update(self, after, before=None, prev_resource=None):
        ...
        yield self._break_if_required(
            self.UPDATE, self.IN_PROGRESS, 'pre-update')
    ...
    def signal(self, details=None):
        ...
        if (details and 'unset_hook' in details and
                valid_hook(details.get('unset_hook')) and
                cfg.CONF.resource_hooks):
