...
class Resource(object):
    ...
    def _break_if_required(self, action, status):
        '''Wait for a breakpoint to clear when one is set.

        If there is a breakpoint for this resource in the stack's environment,
        stop and wait until it's cleared.
        '''
        should_set_breakpoint = self.matches_breakpoint(
            self.stack.root_stack.env.breakpoints)
        if should_set_breakpoint:
            self.state_set(action, status, "Reached breakpoint")
            self.set_breakpoint()
            LOG.info(_LI('Reached breakpoint on %s'), six.text_type(self))
        while self.has_breakpoint() and self.status != self.FAILED:
            try:
                yield
            except Exception:
                self.unset_breakpoint()
                self.state_set(
                    self.INIT, self.COMPLETE,
                    "Something failed while waiting on a breakpoint.")

    def matches_breakpoint(self, breakpoints):
        '''Check whether this resource matches any breakpoint in the list.

        Each breakpoint is a list of all the parent stack names ending with
        the name of the resource.

        E.g. a top-level resource MyServer inside MyStack would look like:

            ['MyStack', 'MyServer']

        A resource within the same stack under NestedStack would be:

            ['MyStack', 'NestedStack', 'AnotherServer']

        We ignore the stack names autogenerated by Heat and instead look only
        at the names that appear within the template.
        '''
        def glob_matches(pattern, string):
            return fnmatch.fnmatchcase(string, pattern)

        root_stack = self.stack.root_stack
        for bp in breakpoints:
            resource_name, stack_names = bp[-1], bp[:-1]
            if not stack_names:
                # At least the root stack must be present:
                continue
            if not glob_matches(resource_name, self.name):
                # The resource name has to match:
                continue
            current_stack = self.stack
            for breakpoint_stack_name in reversed(stack_names):
                if current_stack.parent_resource:
                    if glob_matches(breakpoint_stack_name,
                                    current_stack.parent_resource.name):
                        current_stack = current_stack.parent_resource.stack
                    else:
                        break
                # Don't do glob matching on the root stack:
                elif root_stack == current_stack:
                    if breakpoint_stack_name == current_stack.name:
                        pass
                    else:
                        break
                else:
                ...
                # We went through all the stacks and matched them:
                return True
        # None of the breakpoints had a full match:
        return False

    def has_breakpoint(self):
        # Clear the cache to make sure the data is up to date:
        self._data = None
        bp = self.data().get('breakpoint')
        return bp == "True"

    def set_breakpoint(self):
        self.data_set('breakpoint', "True")

    def unset_breakpoint(self):
        self.data_delete('breakpoint')
    ...
    def create(self):
        ...
        # This method can be called when we replacing a resource, too. In that
        # case, a breakpoint has already been dealt with in `Resource.update`
        # so we shouldn't do it here again:
        if self.stack.action == self.stack.CREATE:
            yield self._break_if_required(self.CREATE, self.IN_PROGRESS)
    ...
    def update(self, after, before=None, prev_resource=None):
        ...
        yield self._break_if_required(self.UPDATE, self.IN_PROGRESS)
    ...
    def signal(self, details=None):
        ...
        # Clear the breakpoint without interfering with resources'
        # `handle_signal` callbacks:
        if (details and 'breakpoint' in details and
                details.get('breakpoint') is False):
            self.unset_breakpoint()
            LOG.info(_LI('Clearing breakpoint on %s'), six.text_type(self))
            return
